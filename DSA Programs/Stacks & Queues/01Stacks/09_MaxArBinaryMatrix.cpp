...
// Function to find largest area in 1d array
...
int maximalRectangle(vector<vector<int>> matrix)
{
    ...
    {
        v.push_back(matrix[0][i]); // Pushing the elements of the first row of given matrix in a vector
    }
    res = largestRectangleArea(v);

    // Now Similarly add the next rows in vector
    for (int i = 1; i < matrix.size(); i++)
    {
        for (int j = 0; j < matrix[0].size(); j++)
        {
            if (matrix[i][j] == 0)
            {
                v[j] = 0; // if base is zero then no building above it will exist
            }
            else
            {
                v[j] += matrix[i][j];
...
int main()
{
    ...
    cout << "Area of maximum rectangle is " << maximalRectangle(matrix);
